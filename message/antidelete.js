...
module.exports = async function connect(caliph, m) {
    ...
    console.log(color(`[CLIENT]`, 'cyan'), color(`${caliph.getName(m.participant} Deleting Message On ${caliph.getName(m.key.remoteJid)} With MESSAGEID :`, 'yellow'), color(m.key.id, 'red'))
